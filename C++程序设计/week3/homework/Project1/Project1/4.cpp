...
private:
	...
	int warriorNum[WARRIOR_NUM]; // 五个战士一组
...
void Warrior::PrintResult(int nTime)
{
	...
	c_str(), it is a C++ thing, not a C one, A common use of c_str (from std::string) is precisely
	to convert a C++ std::string to a const char* C string,
	which is required by many many low level C functions (e.g. Posix system calls like stat, etc).
	*/
}
...
int Headquarter::Produce(int nTime)
{
	...
	while (Warrior::initialLifeValue[makingSeq[color][curMakingSeqIdx]] > totalLifeValue
		...
		/*
		因为totalLifeValue是在不断减少的，退出循环就是战士的生命元大于整个司令部的生命元时，
		但必须去搜寻每个特性的战士，因为有可能前面的战士生命元大于整体，但后面存在战士生命元
		小于整体的情况
		*/
